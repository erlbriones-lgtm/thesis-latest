...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function initLogoCache() {
        try {
            if (window.EMBEDDED_LOGOS) {
                if (window.EMBEDDED_LOGOS.bisu && window.EMBEDDED_LOGOS.bisu.startsWith('data:image/png;base64,iVBORw')) cachedLogos.bisu = window.EMBEDDED_LOGOS.bisu;
                if (window.EMBEDDED_LOGOS.bagongPilipinas && window.EMBEDDED_LOGOS.bagongPilipinas.startsWith('data:image/png;base64,iVBORw')) cachedLogos.bagongPilipinas = window.EMBEDDED_LOGOS.bagongPilipinas;
                if (window.EMBEDDED_LOGOS.tuv && window.EMBEDDED_LOGOS.tuv.startsWith('data:image/png;base64,iVBORw')) cachedLogos.tuv = window.EMBEDDED_LOGOS.tuv;
            }
            if (!cachedLogos.bisu) cachedLogos.bisu = await fetchAsPngDataUrl('/images/BISU.webp');
            if (!cachedLogos.bagongPilipinas) cachedLogos.bagongPilipinas = await fetchAsPngDataUrl('/images/BAGONG-PILIPINAS-LOGO-1-1.png') || await fetchAsPngDataUrl('/images/BAGONG-PILIPINAS-LOGO-1-1.webp');
            if (!cachedLogos.tuv) cachedLogos.tuv = await fetchAsPngDataUrl('public/images/images.webp') || await fetchAsPngDataUrl('/images/images.webp');
        } catch(e) {
    ...
    function printHTMLDocument(title, contentHtml, feedbacks = [], complaints = []) {
        ...
        const bisuLogo = (cachedLogos.bisu && cachedLogos.bisu.startsWith('data:')) ? cachedLogos.bisu : new URL('/images/BISU.webp', window.location.origin).href;
        const bagongPilipinasLogo = (cachedLogos.bagongPilipinas && cachedLogos.bagongPilipinas.startsWith('data:')) 
            ? cachedLogos.bagongPilipinas 
            : new URL('/images/BAGONG-PILIPINAS-LOGO-1-1.png', window.location.origin).href;
        const tuvLogo = (cachedLogos.tuv && cachedLogos.tuv.startsWith('data:')) ? cachedLogos.tuv : new URL('public/images/images.webp', window.location.origin).href;
        ...
        const fullHtml = `
            ...
            <body class="bg-white text-black font-sans">
                <!-- Top Action Toolbar (Sticky in screen preview, completely hidden in print) -->
                <div class="no-print sticky top-0 z-50 bg-[#120042] text-white px-5 py-3 mb-6 rounded-2xl shadow-xl border border-white/20 flex flex-wrap items-center justify-between gap-3 backdrop-blur-md">
                    <div class="flex items-center gap-2.5 text-blue-100">
                        <i class="fa-solid fa-file-signature text-amber-400 text-base"></i>
                        <span class="text-xs sm:text-sm font-bold tracking-wide">${escapeHtml(title)}</span>
                    </div>
                ...
                <div class="printable-letterhead flex flex-col items-center justify-center mb-4 pb-3 border-b border-slate-400 w-full pt-1 text-center font-sans">
                    ...
                    <div class="flex items-center justify-center gap-8 mb-2">
                        <img src="${bisuLogo}" class="logo-header-img" alt="BISU Logo" onerror="this.onerror=null; this.src='images/BISU.webp';" />
                        <img src="${bagongPilipinasLogo}" class="logo-header-img" alt="Bagong Pilipinas Logo" onerror="this.onerror=null; this.src='images/BAGONG-PILIPINAS-LOGO-1-1.png';" />
                        <img src="${tuvLogo}" class="logo-header-img rounded" alt="TÜV Rheinland Logo" onerror="this.onerror=null; this.src='public/images/images.webp';" />
                    </div>
    ...
    async function generateAndDownloadPdfReportFile(filename, subject, bodyStr) {
        try {
            ...
            if (!cachedLogos.bisu) {
                cachedLogos.bisu = await fetchAsPngDataUrl('/images/BISU.webp');
            }
            if (!cachedLogos.bagongPilipinas) {
                cachedLogos.bagongPilipinas = await fetchAsPngDataUrl('/images/BAGONG-PILIPINAS-LOGO-1-1.webp');
            }
            if (!cachedLogos.tuv) {
                cachedLogos.tuv = await fetchAsPngDataUrl('public/images/images.webp') || await fetchAsPngDataUrl('/images/images.webp');
            }
    ...
    function isQrOrOfficeOrKioskUrl() {
        try {
            ...
            const hasOffice = urlParams.has('office') && Boolean(officeParam && officeParam.trim());
            const hasKiosk = urlParams.has('kiosk') || kioskParam === 'true' || kioskParam === '1' || kioskParam === 'all' || modeParam === 'kiosk';
            const hasStation = urlParams.has('station') && Boolean(stationParam && stationParam.trim());
            const hasQr = urlParams.has('qr') || sourceParam === 'qr' || urlParams.get('utm_source') === 'qr' || modeParam === 'qr';
            const isSurveyMode = modeParam === 'feedback' || modeParam === 'qr' || modeParam === 'kiosk' || modeParam === 'complaint' || typeParam === 'complaint';

            const hash = (window.location.hash || '').toLowerCase();
            const isSurveyHash = hash.includes('feedback') || hash.includes('kiosk') || hash.includes('qr') || hash.includes('office');

            const isStoredSession = (sessionStorage.getItem('bisu_scanned_or_office_user') === 'true' || sessionStorage.getItem('bisu_qr_visitor') === 'true') && modeParam !== 'admin' && !hash.includes('admin');

            const result = hasOffice || hasKiosk || hasStation || hasQr || isSurveyMode || isSurveyHash || isStoredSession;
            if (result) {
                try {
                    sessionStorage.setItem('bisu_qr_visitor', 'true');
                    sessionStorage.setItem('bisu_scanned_or_office_user', 'true');
                } catch (e) {}
            }
            return result;
        } catch (e) {
    ...
    acceptConsentBtn.addEventListener('click', () => {
        ...
        setTimeout(() => {
            ...
            privacyModal.classList.remove('flex', 'opacity-0');
            if (viewAdmin) viewAdmin.classList.add('section-hidden');
    ...
    function openAdminView(updateRoute = true) {
        // Strict Guard: If visitor arrived via QR code, office direct link, or kiosk mode,
        // NEVER allow admin view to load. Keep them strictly on the client feedback form.
        if (isQrOrOfficeOrKioskUrl()) {
            console.info('Blocked attempt to open admin view: user is visiting via QR code, office link, or kiosk mode.');
            showDefaultView(false);
            return;
        }

        if (typeof clearAdminCookies === 'function') clearAdminCookies();
    ...
    async function isCurrentUserAdmin(client) {
        // Strict Guard: If visitor arrived via QR code, office direct link, or kiosk mode,
        // they are NEVER authenticated as admin for this session.
        if (isQrOrOfficeOrKioskUrl()) {
            return false;
        }
    ...
    function openAdminLoginModal() {
        if (typeof dismissCookiePrompts === 'function') dismissCookiePrompts();
    ...
    (async () => {
        // Strict Guard: If visitor arrived via QR code, office direct link, or kiosk mode,
        // NEVER auto-open the admin view even if local admin credentials exist on this device.
        if (isQrOrOfficeOrKioskUrl()) {
            showDefaultView(false);
            return;
        }
        ...
        try {
            ...
            if (isAdmin && !isQrOrOfficeOrKioskUrl()) {
                openAdminView();
                fetchAdminData();
            }
            ...
            client.auth.onAuthStateChange(async (_event, session) => {
                ...
                try {
                    ...
                    if (allowed && !isQrOrOfficeOrKioskUrl()) {
                        openAdminView();
                        fetchAdminData();
                    } else {
                        closeAdminView();
...
=====================================================`;
    ...
    function printOfficialComplaintForm(c) {
        ...
        const tuvLogo = (typeof cachedLogos !== 'undefined' && cachedLogos.tuv) ? cachedLogos.tuv : new URL('public/images/images.webp', window.location.origin).href;
        ...
        const printHtml = `
            ...
            <body>
                <table class="header-table">
                    <tr>
                        <td class="logo-cell"><img src="${bisuLogo}" alt="BISU Logo" onerror="this.onerror=null; this.src='images/BISU.webp';"></td>
                        <td class="title-cell">
                        ...
                        <td class="logo-cell" style="width: 20%;">
                            <img src="${bagongPilipinasLogo}" style="max-height: 45px; margin-right: 4px;" alt="Bagong Pilipinas" onerror="this.onerror=null; this.src='images/BAGONG-PILIPINAS-LOGO-1-1.png';">
                            <img src="${tuvLogo}" style="max-height: 45px;" alt="TUV Rheinland" onerror="this.onerror=null; this.src='public/images/images.webp';">
                        </td>
                    </tr>
                </table>
    ...
    async function generateDOCXForDataset(feedbacks = [], complaints = [], title = 'MONTHLY CUSTOMER SATISFACTION SUMMARY REPORT', filenamePrefix = 'BISU_Report') {
        ...
        try {
            ...
            if (!bisuLogo || !bisuLogo.startsWith('data:image/png')) {
                bisuLogo = await fetchAsPngDataUrl('/images/BISU.webp');
                if (bisuLogo) cachedLogos.bisu = bisuLogo;
            }
            if (!bagongPilipinasLogo || !bagongPilipinasLogo.startsWith('data:image/png')) {
                bagongPilipinasLogo = await fetchAsPngDataUrl('/images/BAGONG-PILIPINAS-LOGO-1-1.webp');
                if (bagongPilipinasLogo) cachedLogos.bagongPilipinas = bagongPilipinasLogo;
            }
            if (!tuvLogo || !tuvLogo.startsWith('data:image/png')) {
                tuvLogo = await fetchAsPngDataUrl('public/images/images.webp') || await fetchAsPngDataUrl('/images/images.webp');
    ...
    function generateFeedbackTargetUrl(office, station, kiosk) {
        ...
        // Strictly remove any admin query parameters or hash anchors
    ...
    function renderCurrentQrPreview() {
        ...
        if (qrPreviewUrlText) {
            ...
            qrPreviewUrlText.href = targetUrl;
        }
    ...
    function generatePosterPrintCardHtml(office, station, url, canvasData) {
        ...
        const tuvLogo = cachedLogos.tuv || new URL('public/images/images.webp', window.location.origin).href;
            ...
            <div class="bg-white border-4 border-[#180058] rounded-3xl overflow-hidden shadow-2xl max-w-xl w-full mx-auto text-center flex flex-col items-center relative print:shadow-none print:border-4 print:max-w-none print:w-full page-break-after">
                ...
                <div class="p-8 w-full flex flex-col items-center">
                    ...
                    <div class="flex items-center justify-between border-b-2 border-slate-200 pb-5 mb-6 w-full gap-3">
                        <!-- Left Logo: BISU -->
                        <img src="${bisuLogo}" class="w-20 h-20 object-contain shrink-0" alt="BISU Seal" onerror="this.onerror=null; this.src='images/BISU.webp';">
                        ...
                        <div class="flex items-center gap-2 shrink-0">
                            <img src="${bagongPilipinasLogo}" class="w-16 h-16 object-contain" alt="Bagong Pilipinas" onerror="this.onerror=null; this.src='images/BAGONG-PILIPINAS-LOGO-1-1.png';">
                            <img src="${tuvLogo}" class="w-16 h-16 object-contain rounded" alt="ISO Seal" onerror="this.onerror=null; this.src='public/images/images.webp';">
                        </div>
                    </div>
                    ...
                    <!-- Monospace URL -->
                    <a href="${escapeHtml(url)}" target="_blank" rel="noopener noreferrer" class="text-[10px] font-mono font-semibold text-blue-600 hover:text-blue-800 underline border-t border-slate-200 pt-3 w-full truncate block" title="Open feedback form">${escapeHtml(url)}</a>
                </div>
        ...
        qrs.forEach(qr => {
            ...
            card.innerHTML = `
                <div>
                    ...
                    <a href="${escapeHtml(qr.url)}" target="_blank" rel="noopener noreferrer" class="text-[11px] font-mono text-blue-600 hover:text-blue-800 hover:underline truncate mb-3 px-1 flex items-center gap-1.5 transition-colors cursor-pointer" title="Open feedback link: ${escapeHtml(qr.url)}">
                        <i class="fa-solid fa-arrow-up-right-from-square text-[10px] shrink-0 text-blue-500"></i>
                        <span class="truncate">${escapeHtml(qr.url)}</span>
                    </a>
                </div>
    ...
    function applyRouteFromUrl(isPopState = false) {
        // Strict Guard: If visitor arrived via QR code, office direct link, or kiosk mode,
        // NEVER allow route switching to admin view.
        if (isQrOrOfficeOrKioskUrl()) {
            showDefaultView(false);
            return;
        }
    ...
    function handleUrlQueryParameters() {
        ...
        const isQrOrOfficeVisitor = isQrOrOfficeOrKioskUrl();

        // If the user arrived via QR code, office link, or kiosk, they are NEVER treated as admin for this view.
        const isAdmin = !isQrOrOfficeVisitor && (
            modeParam === 'admin' ||
            window.location.hash.includes('admin') ||
            localStorage.getItem('isLocalAdmin') === 'true' ||
            localStorage.getItem('isLocalOfficeUser') === 'true'
        );

        if (isAdmin && typeof clearAdminCookies === 'function') {
            clearAdminCookies();
        }

        if (isQrOrOfficeVisitor) {
            // Strictly enforce feedback view and guarantee admin view is hidden
            if (viewAdmin) {
        ...
        const isFeedbackKiosk = isQrOrOfficeVisitor || kioskParam === 'true' || kioskParam === '1' || kioskParam === 'all' || modeParam === 'kiosk' || modeParam === 'qr' || modeParam === 'feedback' || Boolean(officeParam);
        const isQrScan = isQrOrOfficeVisitor || urlParams.has('office') || urlParams.has('kiosk') || modeParam === 'qr' || urlParams.has('qr') || urlParams.has('station') || urlParams.get('source') === 'qr' || modeParam === 'feedback';

        // Show privacy modal when user scans QR code or clicks office/kiosk poster link
        if (isQrScan) {
            const privacyModal = document.getElementById('privacy-modal');
            const viewFeedback = document.getElementById('view-feedback');
            const consentCheckbox = document.getElementById('consent-checkbox');
        ...
        if (isFeedbackKiosk || isQrScan || isQrOrOfficeVisitor) {
            const adminLoginBtn = document.getElementById('admin-login-btn');
            const adminLoginBtnMobile = document.getElementById('admin-login-btn-mobile');
            if (adminLoginBtn) adminLoginBtn.classList.add('hidden');
            if (adminLoginBtnMobile) adminLoginBtnMobile.classList.add('hidden');
            ...
            const toggleComplaintBtn = document.getElementById('toggle-complaint-btn');
            if (toggleComplaintBtn && (kioskParam === 'true' || kioskParam === 'all' || modeParam === 'kiosk')) {
        ...
        } else if (modeParam === 'admin' && !isQrOrOfficeVisitor) {
            setTimeout(() => {
    ...
    // ==========================================
    // COOKIE MANAGEMENT & USER DETECTION
    // ==========================================
    function isCurrentUserAdminOrAdminView() {
        // If the user arrived via QR code, office link, or kiosk, they are on public feedback, not admin view
        if (isQrOrOfficeOrKioskUrl()) return false;

        const urlParams = new URLSearchParams(window.location.search);
        const modeParam = urlParams.get('mode');
        const isLocalAdmin = localStorage.getItem('isLocalAdmin') === 'true';
        const isLocalOfficeUser = localStorage.getItem('isLocalOfficeUser') === 'true';
        const hasAdminLayout = document.body.classList.contains('admin-layout');
        const viewAdmin = document.getElementById('view-admin');
        const isAdminViewVisible = viewAdmin && !viewAdmin.classList.contains('section-hidden');
        
        return modeParam === 'admin' || window.location.hash.includes('admin') || isLocalAdmin || isLocalOfficeUser || hasAdminLayout || Boolean(isAdminViewVisible);
    }

    function isUserFromQrOrOfficeLink() {
        if (isCurrentUserAdminOrAdminView()) return false;
        ...
        const hasOfficeParam = Boolean(officeParam && officeParam.trim());
        const hasQrParam = urlParams.has('qr') || sourceParam === 'qr' || modeParam === 'qr' || urlParams.get('utm_source') === 'qr';
        const hasKioskParam = kioskParam === 'true' || kioskParam === '1' || kioskParam === 'all' || modeParam === 'kiosk';
        const hasStationParam = Boolean(stationParam && stationParam.trim());
        const isStoredQrOrOfficeUser = sessionStorage.getItem('bisu_scanned_or_office_user') === 'true';

        return hasOfficeParam || hasQrParam || hasKioskParam || hasStationParam || isStoredQrOrOfficeUser || modeParam === 'feedback' || isQrOrOfficeOrKioskUrl();
    }

    function dismissCookiePrompts() {
    ...
    function clearAdminCookies() {
        deleteCookie('bisu_cookie_consent');
        deleteCookie('bisu_visitor_id');
        deleteCookie('bisu_first_visit');
        deleteCookie('bisu_qr_scanned');
        deleteCookie('bisu_consent_date');
        deleteCookie('bisu_office');
        deleteCookie('bisu_lang');

        localStorage.removeItem('bisu_cookie_consent');
        localStorage.removeItem('bisu_visitor_id');
        localStorage.removeItem('bisu_first_visit');
        localStorage.removeItem('bisu_qr_scanned');
        sessionStorage.removeItem('bisu_scanned_or_office_user');
        sessionStorage.removeItem('bisu_scanned_office');

        dismissCookiePrompts();

        const footerSettingsBtn = document.getElementById('footer-cookie-settings-btn');
        if (footerSettingsBtn) {
            footerSettingsBtn.classList.add('hidden');
        }
    }

    function setCookie(name, value, days = 365) {
        // Never apply cookies to admin or users who did not scan QR / visit office link
        if (isCurrentUserAdminOrAdminView() || !isUserFromQrOrOfficeLink()) {
            return;
        }
        const d = new Date();
    ...
    function initCookieManager() {
        ...
        // 1. If user is Admin or in Admin View, purge and remove cookies completely
        if (isCurrentUserAdminOrAdminView()) {
            clearAdminCookies();
            return;
        }

        // 2. Only apply cookies on the users who scan the QR code or the link to that office
        if (!isUserFromQrOrOfficeLink()) {
            // General / direct visitor without office or QR code
            dismissCookiePrompts();
            if (footerSettingsBtn) {
                footerSettingsBtn.classList.add('hidden');
            }
        ...
        // Confirmed office link or QR code visitor
        sessionStorage.setItem('bisu_scanned_or_office_user', 'true');
        const urlParams = new URLSearchParams(window.location.search);
        const officeParam = urlParams.get('office') || sessionStorage.getItem('bisu_scanned_office') || '';
        if (officeParam) {
            sessionStorage.setItem('bisu_scanned_office', officeParam);
        }

        const isExplicitQr = urlParams.has('qr') || urlParams.get('source') === 'qr' || urlParams.get('mode') === 'qr' || urlParams.has('kiosk');

        // Configure visitor badge
        if (cookieVisitorBadge) {
            if (isExplicitQr) {
                ...
                const label = officeParam.length > 20 ? officeParam.substring(0, 18) + '...' : officeParam;
                cookieVisitorBadge.innerHTML = `<i class="fa-solid fa-building mr-1"></i> Office: ${label}`;
                cookieVisitorBadge.className = 'bg-blue-400 text-slate-900 border border-blue-300 text-[10px] font-black px-2 py-0.5 rounded-full uppercase tracking-wider flex items-center';
            }
        }

        // Show footer cookie settings button only for eligible office / QR visitors
        if (footerSettingsBtn) {
            footerSettingsBtn.classList.remove('hidden');
        }
        ...
        if (!hasConsent) {
            // New user from QR Code or office link who hasn't consented yet
            ...
            // Display cookie banner automatically
            const popupDelay = isExplicitQr ? 300 : 600;
            setTimeout(() => {
                if (cookieBanner && !isCurrentUserAdminOrAdminView() && isUserFromQrOrOfficeLink()) {
        ...
        const saveConsent = (type = 'accepted') => {
            if (isCurrentUserAdminOrAdminView() || !isUserFromQrOrOfficeLink()) return;
        ...
        if (btnOpenModal) {
            btnOpenModal.addEventListener('click', () => {
                if (cookieModal && !isCurrentUserAdminOrAdminView()) cookieModal.classList.remove('hidden');
            });
        }
        ...
        // Persist language selection in cookie on selection change only for QR / office link users
    ...
    // ==========================================
    // CLIENT-SIDE INSPECT & DEVTOOLS PROTECTION
    // ==========================================
    (function initClientSecurityGuards() {
        let lastSecurityToastTime = 0;
        const notifyBlockedAction = (msg) => {
            const now = Date.now();
            if (now - lastSecurityToastTime > 2500) {
        ...
        // 1. Prevent Right-Click Context Menu on user/client side
        window.addEventListener('contextmenu', (e) => {
            if (!isCurrentUserAdminOrAdminView()) {
                e.preventDefault();
                e.stopPropagation();
                notifyBlockedAction('Right-click inspection is disabled on this terminal for security.');
                return false;
            }
        }, { capture: true });

        // 2. Prevent F12 and Inspect / DevTools Keyboard Shortcuts on user/client side
        window.addEventListener('keydown', (e) => {
            if (isCurrentUserAdminOrAdminView()) return;
            ...
            // Inspect shortcuts: Ctrl+Shift+I, Cmd+Alt+I, Ctrl+Shift+J, Cmd+Alt+J, Ctrl+Shift+C, Ctrl+Shift+K
            const isInspectCombo = isCtrlOrMeta && isShiftOrAlt && (
                keyLower === 'i' || keyLower === 'j' || keyLower === 'c' || keyLower === 'k' ||
                e.keyCode === 73 || e.keyCode === 74 || e.keyCode === 67 || e.keyCode === 75
            );

            // View Source (Ctrl+U / Cmd+U) or Save Page (Ctrl+S / Cmd+S)
            const isSourceOrSaveCombo = isCtrlOrMeta && !e.shiftKey && (
                keyLower === 'u' || keyLower === 's' ||
                e.keyCode === 85 || e.keyCode === 83
            );

            if (isF12 || isInspectCombo || isSourceOrSaveCombo) {
                e.preventDefault();
                e.stopPropagation();
                notifyBlockedAction('Developer tools and inspect shortcuts are disabled on this terminal for security.');
                return false;
            }
        }, { capture: true });
    })();
